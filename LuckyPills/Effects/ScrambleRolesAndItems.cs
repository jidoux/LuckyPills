...
internal sealed class ScrambleRolesAndItems(ScrambleRolesAndItemsConfig config) : IPillEffect, IDebugPickPills {
	...
	public void OnEnabled(Player player, int duration) {
		...
		Player currPlayer;
		Player[] playerArray = Player.ReadyList.Where(x => x.IsAlive && x != player).ToArray();
	...
	private static void SetPlayerState(Player currPlayer, PreviousPlayer prevPlayer) {
		currPlayer.SetRoleDelay(prevPlayer.Role, RoleChangeReason.RemoteAdmin, RoleSpawnFlags.None);
...
internal sealed class ScrambleRolesAndItemsConfig {
